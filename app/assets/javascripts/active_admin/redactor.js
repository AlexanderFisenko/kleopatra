		...
		clean: function()
		{
			return {
				...
				encodeEntities: function(str)
				{
					...
					//return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');       //REMOVED FOR EmailTemplate
				},
			...
			if (n.nodeType === 3 && n.parentNode !== 'PRE')
			{
				...
				if (convertUrlLinks && html && matches)
				{
					...
					for (var z = 0; z < len; z++)
					{
						...
						//text = text.replace(/&#36;/g, '$').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');    //  Removed for MailTemplate
